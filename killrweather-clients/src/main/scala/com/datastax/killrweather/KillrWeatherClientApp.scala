...
final class ApiNodeGuardian extends ClusterAwareNodeGuardian with ClientHelper {
 ...
 /* override def preStart(): Unit = {
    ...
    */
    ...
    println("---- Right before Cluster...")
    Cluster(context.system).registerOnMemberUp {
        println("-----> IN Cluster .registerOnMemberUp")
        task = Some(context.system.scheduler.schedule(Duration.Zero, 2.seconds) {
            println("----> Cluster, task: " + task)
            api ! Event.QueryTask
        })
    }
...
private[killrweather] class AutomatedApiActor extends Actor with ActorLogging with ClientHelper {
    ...
    def receive: Actor.Receive = {
        case e: WeatherAggregate =>
            ...
            println("--->Received {} from {}" + e)
        case e: WeatherModel =>
            log.info("--->Received {} from {}", e, sender)
        case Event.QueryTask => 
                log.info("--->Recieved EventQueryTask")
                queries()
    }
    ...
    def queries(): Unit = {
        ...
        println(s"---> initialData: $initialData")
